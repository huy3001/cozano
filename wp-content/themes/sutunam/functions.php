...
function fb_opengraph() {
    ...
    if(is_single()) {
        if(has_post_thumbnail($post->ID)) {
            $img_src = esc_url(wp_get_attachment_image_src(get_post_thumbnail_id( $post->ID ), false)[0]);
        } else {
            $img_src = get_stylesheet_directory_uri() . '/images/logo-thumb.png';
        }
